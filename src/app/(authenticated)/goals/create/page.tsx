...
export default function CreateGoalPage() {
  ...
  const [prefilledData, setPrefilledData] =
    useState<Partial<GoalFormData> | null>(null);
  ...
  const [selectedExistingGoalId, setSelectedExistingGoalId] =
    useState<string>("");
  const [selectedParentGoal, setSelectedParentGoal] = useState("");
  ...
  useEffect(() => {
    if (preselectedType) {
      setFormData((prev) => ({
        ...prev,
        type: preselectedType as GoalFormData["type"],
      }));
    }
  }, [preselectedType]);
    ...
    const loadGoals = async () => {
      try {
        ...
        if (response.data?.success && Array.isArray(response.data.data)) {
          ...
          const nonArchivedGoals = goals.filter(
            (goal) => goal.status !== "archived",
          );
          setExistingGoals(nonArchivedGoals);

          if (selectedExistingGoalId) {
            const selectedGoal = nonArchivedGoals.find(
              (goal) => goal._id === selectedExistingGoalId,
            );
            if (selectedGoal) {
        ...
        >
          <section className="space-y-8 lg:col-span-8">
            <div className="rounded-xl bg-[var(--color-surface-container-low)] p-6 outline outline-1 outline-[color:color-mix(in_srgb,var(--color-outline-variant)_15%,transparent)] sm:p-8">
              <h3 className="mb-6 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--color-text-tertiary)]">
                Core Identity
              </h3>
              ...
              <div className="space-y-6">
                <div className="inline-flex rounded-xl bg-[var(--color-surface-container-lowest)] p-1">
                  <button
                    type="button"
                    onClick={() =>
                      setFormData((prev) => ({ ...prev, isRepetitive: false }))
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setFormData((prev) => ({ ...prev, isRepetitive: true }))
                    }
                    className={`rounded-lg px-5 py-2.5 text-sm font-semibold transition-all ${
                      formData.isRepetitive
                        ? "bg-[var(--color-surface-container-high)] text-[var(--color-primary)]"
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  ...
                  <div
                    className={`rounded-xl bg-[color:color-mix(in_srgb,var(--color-surface-container-high)_60%,transparent)] p-5 ${
          ...
          <aside className="flex flex-col gap-6 lg:col-span-4">
            <div className="rounded-xl bg-[var(--color-surface-container-high)] p-6 outline outline-1 outline-[color:color-mix(in_srgb,var(--color-outline-variant)_15%,transparent)]">
              <h3 className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--color-text-tertiary)]">
                Relational Context
              </h3>
              <label className="mb-2 block text-xs font-semibold text-[var(--color-text-secondary)]">
              ...
              <p className="mt-4 text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-tertiary)]">
                Suggested Parent Goals
              </p>
              ...
              <p className="text-xs leading-relaxed text-[var(--color-text-secondary)]">
                {selectedExistingGoalId ? (
                  <>
                    ...
                    helps AI infer progress patterns from future journal
                    entries.
                  </>
                ) : (
                  <>
                    Choose an existing goal to link hierarchy and progress
                    context.
                  </>
                )}
              </p>
